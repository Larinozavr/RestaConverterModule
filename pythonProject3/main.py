...
def trnsltFl():
    ...
    def MidArcPoint(a,b,c,d,e):
        ...
        M_ = M_star * 0.5
        Q = M_ + MQ
        return f' X{round(float(Q[0]),1)} Y{round(float(Q[1]),1)}'
    ##########################################################
    for i in range(0,len(dxf)):
    ...
    for i in range(0,len(dxf)):
        ...
        if dxf[i] == '42':
            dxf2.append(f'T4{str(MidArcPoint(float(dxf[i-3]),float(dxf[i-1]),float(dxf[i+3]),float(dxf[i+5]), float(dxf[i+1])))}'
            f' X{round(float(dxf[i+3]),1)} Y{round(float(dxf[i+5]),1)}')
    ##########################################################
    dxf2.append('T8')
    dxf = '\n'.join(dxf2)
    dxf = dxf.replace('.0 ',' ')
...
def SaveAs_():
    global dxf
    W = filedialog.asksaveasfile(mode='w', defaultextension=".")
    W.write(dxf)
    W.close()  # `()` was missing.
#######################################################################################################################
